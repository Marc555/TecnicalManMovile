...
class PresupuestoViewModel(
    ...
    // PDF
    fun descargarPresupuestoPdf(context: Context, presupuestoId: Long) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val responseBody = repository.downloadPDF(presupuestoId)
                val fileName = "presupuesto_PPTO${presupuestoId.toString().padStart(4, '0')}.pdf"
                ...
                outputStream.close()
                inputStream.close()
                // Opcional: notifica al usuario en el hilo principal
